...
class Database:
    ...
    def init(self):
        ...
        CREATE TABLE IF NOT EXISTS purchases_accounts (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            telegram_id INTEGER NOT NULL,
            price REAL NOT NULL,
            created_at TEXT NOT NULL
        );
        CREATE TABLE IF NOT EXISTS purchases_stars (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            telegram_id INTEGER NOT NULL,
        ...
        self.conn.commit()
        try:
            self.conn.execute("ALTER TABLE accounts ADD COLUMN price REAL DEFAULT 100")
            self.conn.commit()
    ...
    def get_or_create_user(self, telegram_id, username, first_name):
        ...
        if row:
            ...
            self.conn.commit()
            return dict(self.get_user(telegram_id))
        self.conn.execute(
    ...
    def set_last_start_message(self, telegram_id, message_id):
        self.conn.execute(
            "UPDATE users SET last_start_message_id=? WHERE telegram_id=?",
            (message_id, telegram_id)
        )
        self.conn.commit()

    def change_balance(self, telegram_id, delta):
        self.conn.execute(
    ...
    def get_star_stock(self):
        return int(self.get_setting("star_stock", "0"))

    def set_star_stock(self, n):
        self.set_setting("star_stock", str(n))

    def change_star_stock(self, delta):
        self.conn.execute(
            "UPDATE settings SET value=CAST(value AS INTEGER)+? WHERE key='star_stock'",
            (delta,)
        )
        self.conn.commit()

    def get_star_price(self):
        return float(self.get_setting("star_price", "1"))

    def set_star_price(self, price):
        self.set_setting("star_price", str(price))
    ...
    def get_account_products(self):
        rows = self.conn.execute(
            """SELECT value, price, COUNT(*) AS quantity
               FROM accounts
               WHERE sold=0
    ...
    def get_cheapest_account_price(self):
        row = self.conn.execute(
            "SELECT price FROM accounts WHERE sold=0 ORDER BY id LIMIT 1"
        ).fetchone()
        return float(row["price"]) if row else 0

    def add_account(self, value, price):
        self.conn.execute(
            "INSERT INTO accounts(value,price,sold,created_at) VALUES(?,?,?,?)",
            (value, price, 0, datetime.utcnow().isoformat())
        )
        self.conn.commit()

    def take_account(self):
        row = self.conn.execute(
            "SELECT id,value,price FROM accounts WHERE sold=0 ORDER BY id LIMIT 1"
        ).fetchone()
        if not row:
            return None
        self.conn.execute(
            "UPDATE accounts SET sold=1 WHERE id=?", (row["id"],)
        )
        self.conn.commit()
        return {"value": row["value"], "price": float(row["price"])}

    def add_account_purchase(self, telegram_id, price):
        self.conn.execute(
            "INSERT INTO purchases_accounts(telegram_id,price,created_at) VALUES(?,?,?)",
            (telegram_id, price, datetime.utcnow().isoformat())
        )
        self.conn.commit()

    def add_star_purchase(self, telegram_id, amount, price):
        self.conn.execute(
            "INSERT INTO purchases_stars(telegram_id,amount,price,created_at) VALUES(?,?,?,?)",
            (telegram_id, amount, price, datetime.utcnow().isoformat())
        )
        self.conn.commit()

    def all_telegram_ids(self):
        return [
            r["telegram_id"]
            for r in self.conn.execute("SELECT telegram_id FROM users").fetchall()
        ]

    def create_topup_request(self, telegram_id, payment_type, sent_amount,
                             credited_amount, receipt_file_id, receipt_type):
        cur = self.conn.execute(
            """INSERT INTO topup_requests
            (telegram_id,payment_type,sent_amount,credited_amount,
             receipt_file_id,receipt_type,status,created_at)
            VALUES(?,?,?,?,?,?,?,?)""",
            (
                telegram_id, payment_type, sent_amount, credited_amount,
                receipt_file_id, receipt_type, "pending",
    ...
    def get_topup_request(self, request_id):
        row = self.conn.execute(
            "SELECT * FROM topup_requests WHERE id=?", (request_id,)
        ).fetchone()
        return dict(row) if row else None

    def finish_topup_request(self, request_id, status):
        self.conn.execute(
    ...
    def add_topup(self, telegram_id, amount):
        self.change_balance(telegram_id, amount)

    def stats(self):
        return {
            "users": self.conn.execute("SELECT COUNT(*) FROM users").fetchone()[0],
            "max_id": self.conn.execute(
                "SELECT COALESCE(MAX(internal_id),0) FROM users"
            ).fetchone()[0],
            "account_purchases": self.conn.execute(
                "SELECT COUNT(*) FROM purchases_accounts"
            ).fetchone()[0],
            "stars_sold": self.conn.execute(
                "SELECT COALESCE(SUM(amount),0) FROM purchases_stars"
            ).fetchone()[0],
        }
...
# Telegram file_id of the image used for bot messages. Set this after sending a photo to the bot.
PHOTO_ID = ""
db = Database(DB_PATH)
...
async def send_ui_message(bot: Bot, chat_id: int, body: str, markup=None):
    """Send a text message or a photo+caption when PHOTO_ID is configured."""
    if PHOTO_ID:
        return await bot.send_photo(chat_id, PHOTO_ID, caption=body, reply_markup=markup)
    return await bot.send_message(chat_id, body, reply_markup=markup)

async def edit_ui_message(bot: Bot, chat_id: int, message_id: int, body: str, markup=None):
    """Edit caption for photo messages, or text for normal messages."""
    try:
        if PHOTO_ID:
        ...
        # If the existing message type doesn't match the configured PHOTO_ID,
        # keep the normal fallback behavior.
        if PHOTO_ID:
            return await bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
...
async def safe_edit(call: CallbackQuery, body: str, markup=None):
    try:
        if call.message.photo:
            await call.message.edit_caption(caption=body, reply_markup=markup)
        else:
            await call.message.edit_text(body, reply_markup=markup)
    except Exception:
        await send_ui_message(call.bot, call.message.chat.id, body, markup)
...
async def start(message: Message):
    ...
    sent = await send_ui_message(message.bot, message.chat.id, body, main_kb())
...
async def photoid_command(message: Message):
    ...
    if not message.photo:
        await message.answer(
            "Использование:\n"
            "1. Отправьте мне фотографию с подписью /photoid.\n"
            "2. Я покажу её file_id.\n\n"
            "После этого вставьте file_id в PHOTO_ID в коде."
        )
        return
    ...
    await message.answer(
        "PHOTO_ID:\n\n"
        f"{file_id}\n\n"
        "Вставьте это значение в main.py:\n"
        f'PHOTO_ID = "{file_id}"'
    )
...
async def topup_rubles(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(payment_type="rubles", ui_message_id=call.message.message_id)
    await safe_edit(
        call,
        "Пополнение в рублях\n\n"
        "+79313716777 • Т-банк\n"
...
async def topup_stars(call: CallbackQuery, state: FSMContext):
    await state.set_state(TopupState.amount)
    await state.update_data(payment_type="stars", ui_message_id=call.message.message_id)
    await safe_edit(
        call,
        "Пополнение звездами\n\n"
        "Отправьте звезды на аккаунт @fegote.\n"
...
async def topup_amount(message: Message, state: FSMContext):
    ...
    data["sent_amount"] = sent
    data["credited_amount"] = credited
    ...
    # Fallback only if the original message cannot be edited.
    await message.answer(
        f"{info}\n\n"
        "Теперь отправьте скриншот/чек оплаты фотографией или документом.",
        reply_markup=back_kb()
...
async def topup_approve(call: CallbackQuery):
    ...
    try:
        await call.bot.send_message(
            ...
            f"Ваш баланс: {db.get_user(req['telegram_id'])['balance']:g} ₽"
        )
    except Exception:
        pass
...
def account_product_keyboard():
    products = db.get_account_products()
    rows = []
    for index, product in enumerate(products):
        rows.append([
            InlineKeyboardButton(
                text=f"{product['value']} {product['price']:g}₽",
                callback_data=f"account_product:{index}"
            )
        ])
...
async def accounts(call: CallbackQuery):
    ...
    await safe_edit(
        call,
        f"В наличии: {db.get_account_stock()} аккаунтов\n\nВыберите аккаунт:",
        InlineKeyboardMarkup(inline_keyboard=rows)
    )
    await call.answer()

@router.callback_query(F.data.startswith("account_product:"))
async def account_product(call: CallbackQuery):
    products = db.get_account_products()
    try:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text="Купить",
            callback_data=f"account_buy_product:{index}"
        )],
...
async def account_buy(call: CallbackQuery):
    # Compatibility with an old callback.
    await accounts(call)

@router.callback_query(F.data.startswith("account_buy_product:"))
async def account_buy_product(call: CallbackQuery):
    products = db.get_account_products()
    try:
        index = int(call.data.split(":", 1)[1])
        product = products[index]
    ...
    # Take one exact product/type.
    row = db.conn.execute(
        """SELECT id, value, price
           FROM accounts
           WHERE sold=0 AND value=? AND price=?
           ORDER BY id LIMIT 1""",
        (product["value"], product["price"])
    ).fetchone()

    if not row:
        await safe_edit(call, "Этот товар уже закончился.", back_kb())
        await call.answer()
        return

    db.conn.execute("UPDATE accounts SET sold=1 WHERE id=?", (row["id"],))
    db.conn.commit()
    ...
    await call.message.answer(
        ...
        "Вам напишут в личные сообщения."
    )

    buyer_username = (
        f"@{call.from_user.username}"
        if call.from_user.username else "нет username"
    )
    owner_text = (
        "🛒 Новая покупка;\n\n"
        f"Купили: {row['value']}\n"
        f"Цена: {price:g} ₽\n"
        f"Юз покупателя: {buyer_username}\n"
        f"ID покупателя: {call.from_user.id}\n"
        f"ID в боте: {user['internal_id']}"
    )
    try:
        await call.bot.send_message(OWNER_ID, owner_text)
    except Exception as e:
        logging.warning("Не удалось отправить уведомление о покупке владельцу: %s", e)

    await call.answer("Аккаунт куплен ✅")
...
async def receive_account(message: Message, state: FSMContext):
    ...
    # Expected example:
    # 🇺🇸 +1 100₽ 3 шт
    # We take the final "... 100₽ 3 шт" as price/quantity,
    # everything before it as the account/product name.
    match = re.match(
        r"^(.+?)\s+(\d+(?:[.,]\d+)?)\s*₽\s+(\d+)\s*шт\.?\s*$",
        raw,
        flags=re.IGNORECASE
    )
    ...
    try:
        ...
        if not account or price < 0 or quantity < 1:
            raise ValueError
    except Exception:
        await message.answer("Проверьте цену и количество.")
        return

    # Add the requested quantity to stock.
    ...
    await message.answer(
        ...
        f"Всего аккаунтов в наличии: {db.get_account_stock()} шт.",
...
async def adm_products(call: CallbackQuery):
    ...
    await safe_edit(
        call,
        "🛒 Товары\n\n"
        f"Аккаунты: {db.get_account_stock()} шт.\n"
        f"Звезды: {db.get_star_stock()} шт. • {db.get_star_price():g} ₽/шт.",
        kb
    )
    await call.answer()
...
async def adm_product_accounts(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Добавить аккаунты", callback_data="adm_add_account")],
        [InlineKeyboardButton(text="Назад", callback_data="adm_products")],
    ])
    await safe_edit(
        call,
        "📱 Аккаунты\n\n"
        f"В наличии: {db.get_account_stock()} шт.\n\n"
        "Формат добавления:\n"
        "🇺🇸 +1 100₽ 3 шт",
        kb
    )
...
async def adm_product_stars(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Количество", callback_data="adm_star_stock")],
        [InlineKeyboardButton(text="Цена за 1 шт.", callback_data="adm_star_price")],
        [InlineKeyboardButton(text="Назад", callback_data="adm_products")],
    ])
    await safe_edit(
        call,
        "⭐ Звезды\n\n"
        f"Количество: {db.get_star_stock()} шт.\n"
        f"Цена: {db.get_star_price():g} ₽/шт.",
        kb
    )
    await call.answer()
