...
async def account_buy(call: CallbackQuery):
    ...
    if count <= 0:
        await safe_edit(call, text("stock", count=0), back_kb())
        await call.answer()
        return
    ...
    # Extra safety check after reserving the first item.
    if user["balance"] < price:
        db.conn.execute(
            "UPDATE accounts SET sold=0 WHERE value=? AND sold=1 ORDER BY id DESC LIMIT 1",
            (item["value"],)
        )
        db.conn.commit()
        await safe_edit(
    ...
    # User sees exactly which account was purchased.
    purchase_text = (
        "✅ Покупка оформлена\n\n"
        f"Аккаунт:\n{item['value']}\n\n"
        f"Цена: {price:g} ₽"
    )
    await safe_edit(call, purchase_text, back_kb())

    # Owner receives a separate purchase notification.
    buyer_username = (
